...
// fog:false so neon punches through the smoke instead of being dimmed to nothing by distance —
// the far tubes and signs are the only thing giving the dark end of the lane any shape.
export function neonMat(hex,key='p'){const m=new THREE.MeshBasicMaterial({color:hex,toneMapped:false,fog:false});
...
export const Env={
 ...
 bumperFlash:[0,0],bumperMats:[],softDotTex:null
};
...
export function buildEnvironment(){
 ...
 // Low envMapIntensity is the single biggest reason the lane no longer blows out: a clearcoated
 // surface reflecting a bright room IBL is what turned the whole alley white.
 Env.laneMat=new THREE.MeshPhysicalMaterial({map:laneTexture(),roughness:.24,metalness:.05,clearcoat:.9,clearcoatRoughness:.18,envMapIntensity:.35});
 ...
 const mMat=new THREE.MeshBasicMaterial({color:0xbfd8e2,transparent:true,opacity:.5,toneMapped:false,fog:false});
 const foul=new THREE.Mesh(new THREE.BoxGeometry(LANE_W,0.004,0.025),new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:.85,toneMapped:false,fog:false}));
 ...
 // Dimmer bulbs than a lit alley would use — these read as distant practicals seen through smoke.
 // The visible light shafts under them are built in scene/atmosphere.js (gradient-mapped cones),
 // not here, so they can fade out before touching the lane instead of stacking into a white haze.
 const bulbCol=new THREE.Color(0xffe9c4).multiplyScalar(0.16);
 for(let i=0;i<6;i++){
  const z=1.5+i*3.4;
 ...
 Env.dustPts=new THREE.Points(dg,new THREE.PointsMaterial({map:Env.softDotTex,color:0x8fb4e0,size:0.028,transparent:true,opacity:.22,depthWrite:false,blending:THREE.AdditiveBlending}));
...
export function applyEnv(buildSignsFn,recolorTrail){
 ...
 // Fog is tinted indigo rather than pure black so distance reads as smoky depth, not as a void.
 Scene.scene.fog=new THREE.FogExp2(0x05041a,CFG.lane.fog);
 pinSpot.intensity=44*CFG.lane.pinLight;
 buildSignsFn();
...
export function applyLane(){
 ...
 // Gloss now buys sharper *neon* reflections, not a brighter room reflection — hence the low ceiling.
 Env.laneMat.envMapIntensity=0.12+CFG.lane.gloss*0.3;
...
// Fog is FogExp2, so density is quadratic in distance — anything above ~0.06 erases the pin deck
// and the back-wall signs. Keep presets in the 0.03–0.055 band; smoke depth comes from the haze
// billboards in scene/atmosphere.js, not from cranking this number.
export const ENV_PRESETS={
 'Midnight Alley':{primary:'#8fd8ff',secondary:'#ff9ad5',fog:0.045,brightness:1,anim:'breath'},
 'Cyberpunk':{primary:'#00eaff',secondary:'#ff2bd6',fog:0.04,brightness:1,anim:'pulse'},
 'Synthwave':{primary:'#ff2bd6',secondary:'#7b2bff',fog:0.048,brightness:1.1,anim:'breath'},
 'Fairy Lights':{primary:'#ffd9f2',secondary:'#c9a4ff',fog:0.05,brightness:1.05,anim:'breath'},
 'Deep Space':{primary:'#4a6bff',secondary:'#9fd7ff',fog:0.055,brightness:0.7,anim:'static'},
 'Industrial':{primary:'#ff8a2b',secondary:'#ffd24a',fog:0.05,brightness:0.85,anim:'flicker'},
 '80s Arcade':{primary:'#ffe72b',secondary:'#ff2b3d',fog:0.035,brightness:1.2,anim:'chase'},
 'Blacklight':{primary:'#8a2bff',secondary:'#39ff6a',fog:0.05,brightness:1,anim:'pulse'},
 'Red Room':{primary:'#ff2b3d',secondary:'#ff6a2b',fog:0.055,brightness:0.9,anim:'breath'},
 'Minimal':{primary:'#dfe8ff',secondary:'#8fa3bb',fog:0.03,brightness:0.55,anim:'static'}};
